package com.dariancabot.protek506;
 ...
 *
 * <p>
 * Models the attributes, commands, and measurement of the Protek 506 digital multimeter (DMM).
 * <p>
 * Connection is established using the DMM's RS-232 port and a host system Serial Port using the jSSC library.
 *
 * @author Darian Cabot
 */
public final class Protek506
{
    ...
    //-----------------------------------------------------------------------
    /**
     * Defines the commands that can be sent to the Protek 506 using the {@link #sendCommand(Command) sendCommand} method.
     *
     * <p>
    ...
    public enum Commands
    {
         ...
         * <p>
         * The exception to this procedure is the GO/NO Testing function, which is explained in detail in chapter 5 sections 5-7 of the Protek 506
         * manual.
         */
        MENU(8),
        /**
    ...
    //-----------------------------------------------------------------------
    /**
     * Sends a command to the Protek 506 multimeter over the serial connection.
     *
    ...
    public void disconnectSerialPort()
    {
        if (isConnected)
        {
            try
            {
                ...
                isConnected = false;
                data.value.statistics.setEnabled(false);
            }
    ...
    private boolean connectSerialPort()
    {
        try
        {
            ...
            serialPort.setParams(1200, 7, 2, 0); // Set params
            ...
            isConnected = true;
            data.value.statistics.setEnabled(true);
